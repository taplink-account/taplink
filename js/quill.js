	...
	var Scroll = function (_Parchment$Scroll) {
	  ...
	  _createClass(Scroll, [{
	    ...
	    value: function optimize() {
	      ...
	      if (mutations.length > 0 && this.emitter) {
...
class QuillImageUpload {
	...
	selectLocalImage() {
		...
		input.onchange = () => {
			...
			this.options.parent.$http.request({url: '/pictures/upload', method: 'post', data: formData, headers: {'Content-Type': 'multipart/form-data'}, onUploadProgress: (pe) => {
				console.log(pe);
			}}).then((r) => {
				if (r.data.result == 'success') {
					this.insert('//'+this.options.parent.$account.storage_domain+'/p/'+r.data.response.filename);
				} else if (r.data.result == 'error') {
					this.$dialog.alert({
						message: r.data.error,
	                    confirmText: this.$gettext('Ок'),
	                    type: 'is-danger',
	                    hasIcon: true
					});
				}
...
*/
	...
	sendToServer(file) {
//		alert(file);
		this.insert('https://taplink.ru/s/i/index/screen_viber.png');
	}
