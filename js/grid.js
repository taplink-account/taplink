...
*/
        ...
        plugin.getGridData = function () {
            ...
            return $rows.map(function () {
	            console.log(plugin.getRowData($(this)));
        ...
        plugin.setRowSaved = function ($row) {
            ...
            $row.find("td:first").data("saved", true);
            $row.find("td.selectable:first").html($("<input type=checkbox>"));
        };

        plugin.setRowDirty = function ($row) {
            $row.addClass("sensei-grid-dirty-row").removeClass("sensei-grid-empty-row");
            $row.find("td:first").data("saved", false);
        };
        ...
        plugin.keydown = function (e) {
            ...
            if ((plugin.getActiveCell().length === 0 && !plugin.isEditing && !_.includes(looseCodes, e.which)) ||
                !_.includes(codes, e.which)) {
                return;
            }

            if (!plugin.isEditing && _.includes(plugin.config.disableKeys, e.which)) {
                e.preventDefault();
                return;
            }
            ...
            if (plugin.isEditing && plugin.getEditor() && _.includes(editorCodes, e.which)) {
                return;
            } else {
                e.preventDefault();
            }
...
(function ($) {
  ...
  Editor.extend = function (props) {
    ...
    if (props) {
      child.prototype = $.extend(this.prototype/* child.prototype */, props);
    }
