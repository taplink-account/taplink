...
HTMLHint.addRule({
    id: 'deny-root-text',
    description: 'Text elements must be wrapped in tags',

    init(parser, reporter) {
        var self = this;
        var tagsOpened = 0;

        var selfClosed = mapEmptyTags = parser.makeMap(
        ...
        function onTextNode(event) {
            if (tagsOpened || !/[^\s\t\r\n]/g.test(event.raw)) {
                return;
            }

            reporter.error(
                'Text elements must be wrapped in tags',
                event.line,
                event.col,
                self,
                event.raw
            ...
            // parser.removeListener('text', onTextNode);
        };

        parser.addListener('tagstart', onTagStart);
        parser.addListener('tagend', onTagEnd);
        parser.addListener('text', onTextNode);
    }
});
*/
